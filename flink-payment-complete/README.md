...
- [📚 公共基础知识（两个案例都会用到）](#公共基础知识两个案例都会用到)
  - [命名规范总览](#命名规范总览)
  - [基础知识 A：ProcTime、EventTime、Watermark](#基础知识-aproctimeeventtimewatermark-到底是什么)
  - [基础知识 B：Connector 生态](#基础知识-bflink-tablesql-的-connector-生态)
  - [基础知识 C：消息投递语义](#基础知识-c消息投递语义-at-most-once-at-least-once-exactly-once)
  - [基础知识 D：容错与重启](#基础知识-d容错与重启-作业挂了数据丢不丢)
- [🔵 Case 1：留学缴费首次支付检测](#case-1留学缴费首次支付检测)
  - [Step 1.0：数据源全景](#step-10数据源全景)
  - [Step 1.1：4 个核心作业总览](#step-114-个核心作业总览)
  - [Step 1.2：Kafka Source Table（作业①）](#step-12kafka-source-table作业①)
  - [Step 1.3：流内去重（作业①核心）](#step-13流内去重作业①核心)
  - [Step 1.4：Hive DM 层 T-1 表（作业③）](#step-14hive-dm-层-t-1-表作业③-定时-0200)
  - [Step 1.5：为什么引入 HBase 加速层](#step-15为什么不直接查-hive-引入-hbase-加速层)
  - [Step 1.6：每天同步进 HBase（作业④）](#step-16每天同步进-hbase作业④-定时-0230)
  - [Step 1.7：Lookup Join + 触发发券（作业②）](#step-17flink-查-hbase-判断-触发发券作业②-常驻)
  - [Step 1.8：下游发券服务幂等兜底](#step-18下游发券服务-最后一道幂等兜底)
- [🟢 Case 2：按小时缴费统计大盘](#case-2按小时缴费统计大盘含历史回刷-t-1-校准)
  - [Step 2.0：Watermark 实战 —— TUMBLE 窗口聚合](#step-20watermark-实战-tumble-滚动窗口聚合)
  - [Step 2.1：窗口统计作业独立成作业⑤](#step-21窗口统计作业独立成作业⑤不跟-case-1-核心链路合并)
  - [Step 2.2：历史数据回刷](#step-22历史数据回刷-上线前-1-6月的数据怎么办)
  - [Step 2.3：每天 T-1 校准（作业⑥）](#step-23每天-t-1-校准-修正被-watermark-丢弃的迟到数据作业⑥新增)
  - [Step 2.4：统一写入 ClickHouse](#step-24最终落地方案-为什么统一写-clickhouse怎么写写多频繁)
- [📋 全部作业总览 + 部署 Checklist](#全部作业总览-部署-checklist)
- [附录：全篇面试高频 QA 速查](#附录全篇面试高频-qa-速查)

> 💡 提示：以上链接遵循 GitHub 标准锚点规则
